package com.example.auth_client_starter.properties;

public class AuthProperties {
    private String[] httpUrls = {};

    private RenewProperties renew = new RenewProperties();

    private UrlPermissionProperties urlPermission = new UrlPermissionProperties();

    private Boolean unifiedLogout = false;

    private Boolean isSingleLogin = false;

    private Boolean isShareToken = true;

    private String decryptParamPrivateKey = "";

    private String issuer = "http://localhost:8000";

    public String[] getHttpUrls() {
        return httpUrls;
    }

    public void setHttpUrls(String[] httpUrls) {
        this.httpUrls = httpUrls;
    }

    public RenewProperties getRenew() {
        return renew;
    }

    public void setRenew(RenewProperties renew) {
        this.renew = renew;
    }

    public UrlPermissionProperties getUrlPermission() {
        return urlPermission;
    }

    public void setUrlPermission(UrlPermissionProperties urlPermission) {
        this.urlPermission = urlPermission;
    }

    public Boolean getUnifiedLogout() {
        return unifiedLogout;
    }

    public void setUnifiedLogout(Boolean unifiedLogout) {
        this.unifiedLogout = unifiedLogout;
    }

    public Boolean getIsSingleLogin() {
        return isSingleLogin;
    }

    public void setIsSingleLogin(Boolean isSingleLogin) {
        this.isSingleLogin = isSingleLogin;
    }

    public Boolean getIsShareToken() {
        return isShareToken;
    }

    public void setIsShareToken(Boolean isShareToken) {
        this.isShareToken = isShareToken;
    }

    public String getDecryptParamPrivateKey() {
        return decryptParamPrivateKey;
    }

    public void setDecryptParamPrivateKey(String decryptParamPrivateKey) {
        this.decryptParamPrivateKey = decryptParamPrivateKey;
    }

    public String getIssuer() {
        return issuer;
    }

    public void setIssuer(String issuer) {
        this.issuer = issuer;
    }
}
